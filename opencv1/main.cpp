...
// 缩放图像的函数
cv::Mat scaleImage(const cv::Mat& img, double scale, const cv::Point2f& center, const std::string& interpolation) 
{
    cv::Mat scaled;
    cv::Size outputSize(static_cast<int>(img.cols * scale), static_cast<int>(img.rows * scale));
...
int main(int argc, char** argv)
{
    ...
    // 遍历CSV文件中的每一行
    for (size_t i = 1; i < csvData.size()-5; ++i) 
    { // 从第2行开始，跳过表头

        std::string filename = csvData[i][0];
        double img_scale = std::stod(csvData[i][1]);
        std::string interpolation = csvData[i][2];
        int img_Horizontal = std::stoi(csvData[i][3]);
        int img_Vertical = std::stoi(csvData[i][4]);
        double rotation_angle = std::stod(csvData[i][6]);
        ...
        // 进行缩放变换
         cv::Mat scaledImg = scaleImage(img, img_scale, Center, interpolation);
       ...
       // 计算变换后的图像尺寸
        ...
        // 计算平移后图像的新中心点
        cv::Point2f newCenter(scaledWidth / 2.0f, scaledHeight / 2.0f);
